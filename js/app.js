...
const store = {
    ...
    currentPage: 'dashboard', // Set default ke Dashboard
    currentClassTab: 'Kelas 7 Putri',
    openSubmenu: null // BARU: Menyimpan ID grup menu yang terbuka
}
/* --- CORE FUNCTIONS --- */
// Fungsi baru untuk membuka/menutup sub-menu
window.toggleSubmenu = function(groupId) {
    if (store.openSubmenu === groupId) {
        store.openSubmenu = null; // Tutup jika sudah terbuka
    } else {
        store.openSubmenu = groupId; // Buka menu yang baru
    }
    renderSidebar(); // Render ulang sidebar untuk menerapkan perubahan
}
...
function renderSidebar() {
    ...
    MENUS.forEach(menu => {
        // 1. Jika menu tidak memiliki children (seperti Dashboard)
        if (menu.children.length === 0) {
            const isActive = store.currentPage === menu.id ? 'active' : '';
            html += `
                <div class="nav-item ${isActive}" data-page="${menu.id}" onclick="navigateTo('${menu.id}')">
                    <i class="fa-solid ${menu.icon}"></i>
                    <span>${menu.label}</span>
                </div>
            `;
        } 
        // 2. Jika menu memiliki children (Grup Menu)
        else {
            const isOpen = store.openSubmenu === menu.id;
            const activeGroup = menu.children.some(child => child.id === store.currentPage) ? 'active-group' : '';
            
            // Item Utama (Parent) - bertindak sebagai tombol toggle
            html += `
                <div class="nav-item parent-item ${activeGroup} ${isOpen ? 'open' : ''}" onclick="toggleSubmenu('${menu.id}')">
                    <i class="fa-solid ${menu.icon}"></i>
                    <span>${menu.label}</span>
                    <i class="fa-solid ${isOpen ? 'fa-chevron-up' : 'fa-chevron-down'}" style="margin-left: auto; font-size: 0.7rem;"></i>
                </div>
            `;
            
            // Sub-menu (Children)
            if (isOpen) {
                html += `<div class="submenu-container" id="submenu-${menu.id}">`;
                menu.children.forEach(child => {
                    const isActive = store.currentPage === child.id ? 'active' : '';
                    html += `
                        <div class="nav-item sub-item ${isActive}" data-page="${child.id}" onclick="navigateTo('${child.id}')">
                            <span style="margin-left: 35px;">${child.label}</span>
                        </div>
                    `;
                });
                html += `</div>`;
